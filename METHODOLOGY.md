...
$$D^{\text{shallow}} = -0.05 \quad \text{(5\% drawdown threshold)}$$

$$D^{\text{moderate}} = -0.15 \quad \text{(15\% drawdown threshold)}$$
...
\mathbf{U}_z \mathbf{h}_{t-1} + \mathbf{b}_z\right)$$
...
where $\sigma(\cdot)$ is the sigmoid activation and $\odot$ denotes
...
|---|---|---|---|
| 1 | GRU | 32 | Tanh / Sigmoid (internal) |
| 2 | Dense | 16 | ReLU |
| 3 | Dense | 1 | Linear |
...
where $\hat{\mathbf{p}}_{t+1} \in \mathbb{R}^4$ with
$\sum_k \hat{p}_{t+1,k} = 1$.

### 8.2 Architecture

| Layer | Type | Units | Activation |
|---|---|---|---|
| 1 | GRU | 64 | Tanh / Sigmoid (internal) |
| 2 | Dropout | — | Rate = 0.2 |
| 3 | Dense | 32 | ReLU |
| 4 | Dense | 4 | Softmax |
...
in steps of 0.05, crossed with seven candidate persistence windows from
...
|---|---|---|
| `data/` | All `.pkl` output files, `.json` config | `.gitignore` — not pushed |
...
### 14.4 Known Technical Debt

The GRU volatility forecast ($\hat{\sigma}^{\text{GRU}}$) is not
currently stored in `features.pkl` as a live production feature. It is
computed and visualized independently but is not fed back into the regime
classifier's input feature set. Full integration of the two-stage
architecture — where the GRU volatility output serves as a real-time
input to the regime classifier — is identified as a future enhancement.
...
| $S_t$ | Combined stress probability: $\hat{p}_{t,2} + \hat{p}_{t,3}$ |
